...
_('Belum ada aturan terjadwal, klik "Tambahkan Aturan" untuk mulai mengatur.')+'</div>';}
...
_('Belum ada aturan terjadwal, klik "Tambahkan Aturan" untuk mulai mengatur.')+'</div>';return;}
...
var updateBadge=document.getElementById('bandix-update-badge');if(updateBadge){if(hasUpdate){updateBadge.style.display='inline-block';updateBadge.onclick=function(){window.location.href='/cgi-bin/luci/admin/network/bandix/settings';};updateBadge.title=_('Update available, click to go to settings');}else{updateBadge.style.display='none';}}}).catch(function(err){console.debug('Failed to check update:',err);});},500);})();return view;}});